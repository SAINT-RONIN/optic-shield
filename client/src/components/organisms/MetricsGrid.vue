...
const metrics = computed(() => {
  ...
  const avgLum = a?.luminanceMetrics.length
    ...
    : 0
  const motionPeak = a?.motionMetrics.length
    ? Math.max(...a.motionMetrics.map(m => m.motionIntensity))
    : 0
  const sceneCuts = a?.sceneCutMetrics.length
    ? a.sceneCutMetrics.filter(m => m.cutConfidence > 0.8).length
    : 0
  return [
    ...
    { label: 'MOTION', value: motionPeak.toFixed(2), subtitle: 'intensity (peak)', color: '#00C9A7' },
    { label: 'SCENE CUTS', value: String(sceneCuts), subtitle: 'detected (conf >0.8)', color: '#5B8DEF' }
  ]
})
</script>
...
<template>
  <div class="grid grid-cols-2 gap-2.5">
    <MetricCard
      ...
      :label="m.label" :value="m.value" :subtitle="m.subtitle" :color="m.color"
    />
  </div>
</template>
